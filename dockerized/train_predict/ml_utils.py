...
def spark_retrain_model(data):
    ss = SparkSession.builder.appName("capstone").getOrCreate()
    pdf = read_from_mongo()
    sdf = ss.createDataFrame(pdf[['category','text']])
    print(sdf.count())
    lem_udf = udf(retrain_model, ArrayType(StringType()))
    sdf = sdf.withColumn('processed_text',lem_udf(sdf['text']))
    print(sdf.head(2))
    return sdf

def spark_predict_category(data):
    ss = SparkSession.builder.appName("capstone").getOrCreate()
    pdf = read_from_mongo()
    sdf = ss.createDataFrame(pdf[['category','text']])
    print(sdf.count())
    lem_udf = udf(predict_category, ArrayType(StringType()))
    sdf = sdf.withColumn('processed_text',lem_udf(sdf['text']))
    print(sdf.head(2))
    return sdf
